...
const continueLevelButton = document.querySelector("#continueLevelButton");
...
function renderScenario() {
  ...
  mixChoicesForScenario(scenario.choices, currentScenario).forEach((choice) => {
...
function showLevelIntro() {
  const level = getCurrentLevel();
  updateLevelMap(level.index);
  levelBadge.textContent = `Level ${level.index} Complete - Level ${level.index + 1} Unlocked`;
  levelTitle.textContent = level.name;
  levelCopy.textContent = level.copy;
  continueLevelButton.textContent = `Start Level ${level.index + 1}`;
  showScreen(levelScreen);
}

function endGame() {
  progressFill.style.width = "100%";
  updateLevelMap(levels.length - 1);
...
function mixChoicesForScenario(choiceList, scenarioIndex) {
  const correctChoice = choiceList.find((choice) => choice.impact > 0);
  const otherChoices = shuffleChoices(choiceList.filter((choice) => choice.impact <= 0));
  const correctPositions = [1, 2, 0, 2, 1];
  const correctPosition = correctPositions[scenarioIndex % correctPositions.length];
  otherChoices.splice(correctPosition, 0, correctChoice);
  return otherChoices;
}

function shuffleChoices(choiceList) {
  const mixedChoices = [...choiceList];

  for (let index = mixedChoices.length - 1; index > 0; index -= 1) {
    const randomIndex = Math.floor(Math.random() * (index + 1));
    [mixedChoices[index], mixedChoices[randomIndex]] = [mixedChoices[randomIndex], mixedChoices[index]];
  }

  return mixedChoices;
}

function updateLevelMap(activeLevelIndex) {
  levelNodes.forEach((node, index) => {
    node.classList.toggle("active", index === activeLevelIndex);
    node.classList.toggle("complete", index < activeLevelIndex);
  });
}
